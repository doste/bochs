...
void bx_dbg_info_tss_tr_command(void)
{
  bx_dbg_sreg_t tr;
...
///////////////////
/*
  le cambio el nombre a bx_dbg_info_tss_command. esa funcion sera llamada para imprimir la TSS que tiene el TR, asique se mantendra igual.
  le cambio el nombre a bx_dbg_info_tss_tr_command y la que defino a continuacion sera la nueva bx_dbg_info_tss_command, la cual recibira un parametro
  opcional que indique el indice en la GDT de la TSS a imprimir:

*/
void bx_dbg_info_tss_command(unsigned idx) {
  // idx sera un selector de la GDT
  // asique necesitamos extraer la Base y el Segment Limit de este
  bx_dbg_global_sreg_t gdtr;
  BX_CPU(dbg_cpu)->dbg_get_gdtr(&gdtr);
  bool show_only_tr = 0;

  //dbg_printf("ACA! idx:%u\n", idx);

  //dbg_printf("(unsigned) EMPTY_ARG: %u\n", (unsigned) EMPTY_ARG);

  if (idx == (unsigned) EMPTY_ARG) {
    // si el argumento es vacio. muestro la tss apuntada por el TR
    // (como estaba ya implementado asique reuso la funcion:)

    //dbg_printf("ENTRO!!! idx:%u\n", idx);

    bx_dbg_info_tss_tr_command();
    show_only_tr = 1;
    ...
    if (bx_dbg_read_linear(dbg_cpu, gdtr.base + 8*idx, 8, entry)) {
      ...
      //Bit32u limit = (hi & 0x000f0000) | (lo & 0xffff);
      Bit32u len = (lo & 0xffff) + 1;

      unsigned s = (hi >> 12) & 0x1;

      unsigned type = (hi >> 8) & 0xf;

      // antes de imprimir algo, chequeo que el idx del parametro sea un indice valido
      // osea que apunte a un descriptor de TSS:

      if (!s) { // los descriptores de TSS son Descriptores de Sistema, por lo tanto tiene S=0
          switch (type) {
            case BX_SYS_SEGMENT_AVAIL_286_TSS:
            case BX_SYS_SEGMENT_BUSY_286_TSS:
            case BX_SYS_SEGMENT_AVAIL_386_TSS:
            case BX_SYS_SEGMENT_BUSY_386_TSS:
              //dbg_printf("at 0x%08x, length 0x%05x", base, limit);
              //dbg_printf("SI! type: %u\n", type);
              break;
            default:
              // task, int, trap, or call gate.
              dbg_printf("GDT[0x%04x] is not a TSS Descriptor \n", idx << 3);
              return;
              //break;
          } 
      } else {
      ...
      // hago otro chequeo: si el idx que me pasaron es justo el de la TSS que esta en el TR,
      // muestro algo que indique eso:
      bx_dbg_sreg_t tr;
      BX_CPU(dbg_cpu)->dbg_get_tr(&tr);

      bx_address base_tr = (tr.des_l>>16) |
                       ((tr.des_h<<16)&0x00ff0000) | (tr.des_h & 0xff000000);


      if (base == base_tr) {
          dbg_printf("Task State Segment (TR) : GDT[0x%04x] (base= 0x" FMT_ADDRX ")\n",
                          idx << 3, base);
      } else {
          dbg_printf("Task State Segment : GDT[0x%04x] (base= 0x" FMT_ADDRX ")\n",
                          idx << 3, base);
      }

      //dbg_printf("GDT[0x%04x]=", idx << 3);
  ...
  if (show_only_tr)
    dbg_printf("You can list individual entries with 'info tss [NUM]'\n");
}
